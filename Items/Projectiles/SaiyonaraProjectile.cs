...
{
	public class SaiyonaraProjectile : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Sai");
		}
		...
		public override bool TileCollideStyle(ref int width, ref int height, ref bool fallThrough)
		{
			...
			width = height = 15; // notice we set the width to the height, the height to 10. so both are 10
			return true;
		}
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is SaiyonaraProjectile daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
