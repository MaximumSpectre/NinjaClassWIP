...
{
	public class SaiyonaraProjectile2 : ModProjectile
	{
		public int duration = 200;                // the time the projectile stays in the air
		public int penetration = 3;             // how many eneemies the projectile penetrate
		public const float drag = 0.995f;            // the drag of the projectile
		...
		private const int MAX_TICKS = 6;        // how long untill gravity is turned on
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is SaiyonaraProjectile2 daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
