...
namespace NinjaClass.Projectiles
{
    ...
    {
        public int duration = 35;                // the time the projectile stays in the air
        public int penetration = 2;             // how many eneemies the projectile penetrate
        public const float drag = 0.98f;            // the drag of the projectile
        public const float gravity = 0.2f;      // the gravity of the projectile
        ...
        public int killDust = 9;                   // which dust used when it dies
        ...
        public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            ...
            if (Main.rand.Next(6) == 0) // the chance
            {
                target.AddBuff(BuffType<Buffs.Wound>(), 30);
            }
        ...
        private void UpdateStickyJavelins(NPC target)
        {
            ...
            for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
            {
                ...
                if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
                    ...
                    && currentProjectile.modProjectile is CopperDaggerProjectile daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
