...
{
	public class SearingEdgeProjectileMega : ModProjectile
	{
		...
		public override bool TileCollideStyle(ref int width, ref int height, ref bool fallThrough)
		{
			...
			width = height = 14; // notice we set the width to the height, the height to 10. so both are 10
			return true;
		}
		...
		public override void Kill(int timeLeft)
		{
			...
			{
				int ekuslosion = Projectile.NewProjectile(projectile.position.X, projectile.position.Y, 0, 0, mod.ProjectileType("SearingEdgeProjectileMega2"), truDamage/ 2, 0, Main.myPlayer, 0f, 0f);
				Main.projectile[ekuslosion].width = 250;
				Main.projectile[ekuslosion].height = 250;
				Vector2 center;
				float angle;
				Vector2 tar;
				for (int i = 0; i < 4+Main.rand.Next(3); i++)
				{
					...
					int roseFire = Projectile.NewProjectile(center.X, center.Y, tar.X, tar.Y, 15, (int)(truDamage * 0.20f), projectile.knockBack, projectile.owner, 0f, 0f);
					Main.projectile[roseFire].thrown = true;
					Main.projectile[roseFire].magic = false;
					Main.projectile[roseFire].penetrate = 3;
					Main.projectile[roseFire].usesLocalNPCImmunity = true;
					Main.projectile[roseFire].localNPCHitCooldown = 30;
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is SearingEdgeProjectile daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
