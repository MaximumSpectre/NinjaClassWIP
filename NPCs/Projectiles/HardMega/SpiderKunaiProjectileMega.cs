...
namespace NinjaClass.Projectiles.HardMega
{
	public class SpiderKunaiProjectileMega : ModProjectile
	{
		public int duration = 128;                // the time the projectile stays in the air
		...
		public int killDust = 7;                   // which dust used when it dies
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is SpiderKunaiProjectileMega daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
