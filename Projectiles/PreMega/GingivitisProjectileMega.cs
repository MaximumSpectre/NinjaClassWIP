...
{
	public class GingivitisProjectileMega : ModProjectile
	{
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
		{
			...
			if (firstHit)
			{
				...
				for (int i = 0; i < 1; i++)
				{
					...
					tar.Normalize();
					tar *= 14; // speed
					Projectile.NewProjectile(center.X, center.Y, tar.X, tar.Y, mod.ProjectileType("GingivitisProjectileMega2"), (int)(initialDamage), knockback, projectile.owner, 0f, 0f);
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is GingivitisProjectileMega daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
