...
namespace NinjaClass.Projectiles.PreMega
{
	public class WoodenDaggerProjectileMega : ModProjectile
	{
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is WoodenDaggerProjectileMega daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
		...
		{
			Texture2D texture = mod.GetTexture("Projectiles/PreMega/WoodenDaggerProjectileMega_Glow");
			spriteBatch.Draw
			(
				texture,
				projectile.position,
