...
{
	public class SilverDaggerProjectileMega : ModProjectile
	{
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is SilverDaggerProjectileMega daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
		...
		{
			Texture2D texture = mod.GetTexture("Projectiles/PreMega/SilverDaggerProjectileMega_Glow");
			spriteBatch.Draw
			(
				texture,
				projectile.position,
