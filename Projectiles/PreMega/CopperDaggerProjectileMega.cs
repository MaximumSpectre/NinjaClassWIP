...
{
	public class CopperDaggerProjectileMega : ModProjectile
	{
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
		{
			...
			target.AddBuff(BuffType<Buffs.Wound>(), 40);
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is CopperDaggerProjectileMega daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
		...
		{
			Texture2D texture = mod.GetTexture("Projectiles/PreMega/CopperDaggerProjectileMega_Glow");
			spriteBatch.Draw
			(
				texture,
				projectile.position,
