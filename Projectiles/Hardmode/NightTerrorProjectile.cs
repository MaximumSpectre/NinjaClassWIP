...
{
	public class NightTerrorProjectile : ModProjectile
	{
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
		{
			...
			if (hasSpawned == false)
			{
				...
				for (int i = 0; i < 1; i++)
				{
					...
					tar.Normalize();
					tar *= 13; // speed
					Projectile.NewProjectile(center.X, center.Y, tar.X, tar.Y, mod.ProjectileType("NightTerror2Projectile"), (int)(damage * 0.50f), knockback, projectile.owner, 0f, 0f);
				}  // last 0f,0f, this is projectile.ai[0] and projectile.ai[1] where you can put timer, angle, or the target.
			}
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is NightTerrorProjectile daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
		...
		public override void AI()
		{
			...
			Lighting.AddLight(projectile.Center, 0.62f, 0.23f, 0.72f); // R G B values from 0 to 1f. This is the red from the Crimson Heart pet
		}
