...
namespace NinjaClass.Projectiles.Hardmode.Kunai
{
	...
	{
		public int duration = 96;                // the time the projectile stays in the air
		public int penetration = 8;             // how many eneemies the projectile penetrate
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is SpiderKunaiProjectile daggerProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
